...
Vector2 get_food() {
  return (Vector2){(float)GetRandomValue(0, WIDTH / SQUARE_SIZE - 1),
                   (float)GetRandomValue(0, HEIGHT / SQUARE_SIZE - 1)};
}
...
void handle_collisions(game_context *gc) {
  ...
  Rectangle snake_head = {snake.x * SQUARE_SIZE, snake.y * SQUARE_SIZE,
                          SQUARE_SIZE, SQUARE_SIZE};

  Rectangle food_rect = {gc->food.x * SQUARE_SIZE, gc->food.y * SQUARE_SIZE,
                         SQUARE_SIZE, SQUARE_SIZE};
  ...
  if (snake.x < 0 || snake.x >= WIDTH / SQUARE_SIZE || snake.y < 0 ||
      snake.y >= HEIGHT / SQUARE_SIZE) {
    gc->game_over = true;
  }
...
void draw_game(game_context gc) {
  if (gc.game_over) {
    ...
    DrawRectangle(gc.food.x * SQUARE_SIZE, gc.food.y * SQUARE_SIZE, SQUARE_SIZE,
                  SQUARE_SIZE, RED);

    for (size_t i = 0; i < gc.snake_len; i++)
      DrawRectangle(gc.snake[i].x * SQUARE_SIZE, gc.snake[i].y * SQUARE_SIZE,
                    SQUARE_SIZE, SQUARE_SIZE, DARKGREEN);
  }
}
